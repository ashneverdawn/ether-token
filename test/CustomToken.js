...
//CREATION
    ...
    it("Creation: Should create initial balance of 4096 for creator", function(done) {
        CustomToken.new(4096, 'Test Token', {from: accounts[0]}).then(function(ctr) {
            return ctr.balanceOf.call(accounts[0]);
    }).then(function (result) {
        assert.strictEqual(result.toNumber(), 4096);
        done();
        }).catch(done);
    });
    ...
    it("Creation: Should succeed in creating over 2^256 - 1 (max) tokens", function(done) {
        //2^256 - 1
        CustomToken.new('115792089237316195423570985008687907853269984665640564039457584007913129639935', 'Test Token', {from: accounts[0]}).then(function(ctr) {
...
//normal transfers without approvals.
    ...
    it("transfers: ether transfer should be reversed.", function(done) {
        var ctr;
        CustomToken.new(10000, 'Test Token', {from: accounts[0]}).then(function(result) {
            ctr = result;
